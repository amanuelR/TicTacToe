...
open class TicTacToe {
    ...
    protected fun checkRows(): Int {
        for (row in 0 until SIDE) if (game[row][0] != 0 && game[row][0] == game[row][1] && game[row][1] == game[row][2]) return game[row][0]
        return 0
    }

    protected fun checkColumns(): Int {
        for (col in 0 until SIDE) if (game[0][col] != 0 && game[0][col] == game[1][col] && game[1][col] == game[2][col]) return game[0][col]
        return 0
    }

    protected fun checkDiagonals(): Int {
        if (game[0][0] != 0 && game[0][0] == game[1][1] && game[1][1] == game[2][2]) return game[0][0]
        return if (game[0][2] != 0 && game[0][2] == game[1][1] && game[1][1] == game[2][0]) game[2][0] else 0
    }
